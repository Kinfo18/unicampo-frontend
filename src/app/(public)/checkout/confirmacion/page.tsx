...
import { CheckCircleIcon, ClipboardDocumentListIcon, ShoppingBagIcon } from '@heroicons/react/24/outline';
...
interface OrderDetail {
  ...
  createdAt: string;
  items: { product: { name: string; price: string }; quantity: number; unitPrice: string }[];
}
...
export default function ConfirmacionPage() {
  ...
  useEffect(() => {
    if (!orderId) return;
    api
      .get(`/orders/${orderId}`, { headers: { Authorization: `Bearer ${token}` } })
        ...
        {order && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            ...
            <div className="border-t border-gray-100 pt-3 flex justify-between">
              ...
              <span className="font-extrabold text-primary-700">{formatPrice(Number(order.total))}</span>
            </div>
