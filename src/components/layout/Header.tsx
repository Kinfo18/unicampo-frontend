      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Link href="/carrito" className="relative p-2 text-gray-500 hover:text-primary-600 transition-colors">
          ...
          <div className="border-t border-gray-100 pt-3">
            {mounted && isAuthenticated ? (
              <div className="flex flex-col gap-2">
                <Link href="/cuenta" onClick={() => setMobileOpen(false)} className="text-sm text-gray-700">
                  Mi cuenta — {user?.name}
                </Link>
                {user?.role === 'ADMIN' && (
                  <Link href="/admin" onClick={() => setMobileOpen(false)} className="text-sm text-primary-600 font-medium">
                    Panel de administración
                  </Link>
                )}
                <button onClick={clearAuth} className="text-sm text-red-500 text-left">
                  Cerrar sesión
                </button>
              </div>
            ) : (
